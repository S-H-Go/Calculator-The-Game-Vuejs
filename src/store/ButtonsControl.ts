import { SEButtonFactory, buttonFactory, conversationButtonFactory } from './buttonFactory'
import type { ILevelData } from './interface'
...
export class ButtonControl {
  ...
  updateButtonsList(levelButtons: string[]) {
    ...
    const buttons = [['', '', ''], ['', '', ''], ['', '', '']] as any[][]
    const iIndex = [0, 1, 1, 2, 2]
    const jIndex = [1, 1, 2, 1, 2]
    let p = 0
  ...
  eachButton(operation: string, num: number) {
    switch (operation) {
      case '+': {
        for (const i in this.levelButtons) {
