...
export function buttonFactory(index: string, num1?: number, num2?: number, operation?: string) {
  switch (index) {
    case 'insert': {
      return { tag: markRaw(InsertButton), num1 }
    }
    case 'replace': {
      return { tag: markRaw(ReplaceButton), num1, num2 }
    }
    case 'each': {
      return { tag: markRaw(EachButton), operation, num1 }
    }
    case 'elementaryArithmetic': {
      return { tag: markRaw(ElementaryArithmetic), operation, num1 }
    }
  }
}
export function conversationButtonFactory(info: string) {
  return { tag: markRaw(ConversationButton), info }
}
